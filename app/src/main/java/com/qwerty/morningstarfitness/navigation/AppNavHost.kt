...
fun AppNavHost(modifier: Modifier = Modifier, navController: NavHostController = rememberNavController(), startDestination: String = ROUTE_SPLASH) {
    ...
    fun goHome(scope: kotlinx.coroutines.CoroutineScope) { scope.launch { memberViewModel.refreshFromFirebase(); attendanceViewModel.fetchAttendanceHistory(); navController.navigate(ROUTE_HOME) { popUpTo(ROUTE_ENTRY) { inclusive = true } } } }

    NavHost(navController = navController, startDestination = startDestination, modifier = modifier) {
        composable(ROUTE_ENTRY) { EntryScreen(onCreateAccount = { navController.navigate(ROUTE_REGISTRATION) }, onLogin = { navController.navigate(ROUTE_LOGIN) }, onEnterGym = { navController.navigate(if (authViewModel.currentUser() != null) ROUTE_SCAN_ENTRY else ROUTE_LOGIN) }) }
        ...
        composable(ROUTE_SCAN_ENTRY) {
            val scope = rememberCoroutineScope(); var isRecording by remember { mutableStateOf(false) }; var checkInMessage by remember { mutableStateOf<String?>(null) }
            LaunchedEffect(Unit) { memberViewModel.refreshFromFirebase() }
            QrEntryScreen(qrCodeValue = memberViewModel.ensureMembershipQr(), fullName = memberViewModel.memberForm?.fullName, memberId = memberViewModel.memberId, status = memberViewModel.getMembershipStatus(), membershipExpiry = memberViewModel.membershipExpiry, isRecording = isRecording, checkInMessage = checkInMessage, onBack = { navController.popBackStack() }, onPasswordEntry = { navController.navigate(ROUTE_PASSWORD_ENTRY) }, onRecordCheckIn = { if (!isRecording) scope.launch { if (authViewModel.currentUser() == null) { checkInMessage = "Your session has expired. Please log in again."; return@launch }; isRecording = true; val recorded = attendanceViewModel.recordCheckIn(); checkInMessage = when { recorded -> { attendanceViewModel.fetchAttendanceHistory(); "Check-in recorded for today." }; attendanceViewModel.lastCheckInError != null -> attendanceViewModel.lastCheckInError ?: "Could not save the visit."; else -> "You are already checked in today." }; isRecording = false } })
        }
        composable(ROUTE_PASSWORD_ENTRY) {
            ...
            PasswordEntryScreen(memberName = memberViewModel.memberForm?.fullName, isVerifying = isVerifying, errorMessage = errorMessage, onBack = { navController.popBackStack() }, onVerify = { password -> scope.launch { isVerifying = true; errorMessage = null; if (authViewModel.verifyCurrentUserPassword(password)) { val recorded = attendanceViewModel.recordCheckIn(); if (recorded) { attendanceViewModel.fetchAttendanceHistory(); navController.navigate(ROUTE_HOME) { popUpTo(ROUTE_PASSWORD_ENTRY) { inclusive = true } } } else errorMessage = attendanceViewModel.lastCheckInError ?: "You are already checked in today." } else errorMessage = "Incorrect password. Please try again."; isVerifying = false } })
        }
        ...
        composable(ROUTE_PLAN) { PlanScreen(onBack = { navController.popBackStack() }, onContinue = { memberViewModel.updateSelectedPlan(it); navController.navigate(ROUTE_PAYMENT) }) }
        ...
        composable(ROUTE_RENEW_PAYMENT) {
            PaymentScreen(plan = memberViewModel.selectedPlan, paymentMethod = memberViewModel.paymentMethod, isProcessing = paymentViewModel.isProcessing || memberViewModel.isRenewing, errorMessage = paymentViewModel.errorMessage ?: memberViewModel.renewalError, onMethodChange = { memberViewModel.updatePaymentMethod(it) }, onBack = { navController.popBackStack() }, onSimulateSuccess = {
                val plan = memberViewModel.selectedPlan ?: return@PaymentScreen
                paymentViewModel.simulateSuccessfulPayment(amount = plan.priceKsh, purpose = "membership_renewal", referenceId = "RENEW-${UUID.randomUUID()}", planId = plan.id, planLabel = plan.label, planDuration = plan.durationMonths) { success -> if (success) memberViewModel.renewMembership(plan) { renewed -> if (renewed) navController.navigate(ROUTE_HOME) { popUpTo(ROUTE_HOME) { inclusive = true } } } }
            }, onSimulateCancel = { paymentViewModel.simulateCancelledPayment { } })
        }
